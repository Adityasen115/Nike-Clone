  ...
  return (
    <footer className="max-container">
      <div className="flex justify-between items-start gap-20 flex-wrap max-lg:flex-col">
        ...
        <div className="flex flex-1 justify-between lg:gap-10 gap-20 flex-wrap text-white-400 font-montserrat">
              {footerLinks.map((Section) => (
                <div key={Section}>
                  <h4 className="text-2xl font-medium  leading-normal mb-6">{Section.title}</h4>
                  <ul>
                    {Section.links.map((link) => (
                      <li key={link.name} className="mt-3 text-base leading-normal hover:text-slate-gray"><a href={link.link}>{link.name}</a></li>
                    ))}
                  </ul>
                </div>
              ))}
      ...
      <div className="flex justify-between text-white-400 mt-24 max-sm:flex-col max-sm:items-center">
        <div className="flex flex-1 justify-start items-center gap-2 font-montserrat cursor-pointer">
          ...
          <p>Copyright. All rights reseerved</p>
        </div>
